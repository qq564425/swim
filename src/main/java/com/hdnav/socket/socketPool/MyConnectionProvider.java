package com.hdnav.socket.socketPool;
...
public class MyConnectionProvider implements ConnectionProvider {
	...
	private static Object[] object_lock_list = new Object[Integer.parseInt(ResourceBundle.getBundle("com.hdnav.socket.socketPool.config").getString("max_size"))];
	private String ip;  
	private String port;   
	...
	*/  
	private MyConnectionProvider() throws UnknownHostException, IOException {  
		ResourceBundle  res =ResourceBundle.getBundle("com.hdnav.socket.socketPool.config");
	...
	*/  
	...
	public boolean closeConnectionByAddress(String address) {
		boolean flag = false; 
		for (int i = 0; i < socketpool.length; i++) { 
			if (socketpool[i] != null) {
				if (address.equals(((InetSocketAddress)socketpool[i].getLocalSocketAddress()).toString())) {
					try {
						socketpool[i].shutdownInput();
						Thread.sleep(1000);
						socketpool[i].shutdownOutput();
						Thread.sleep(1000);
						socketpool[i].close();
						flag = true;
					} catch (Exception e) {
